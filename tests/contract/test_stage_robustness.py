...
def test_builtin_stage_threads(client, tmp_path, monkeypatch):
    """identify and embed on the single model thread, jpg and aesthetics on the CPU pool: pinned in the manifests
    ...
    events(client.post("/run", json={"inputs": [{"path": make_jpg(tmp_path / "a.jpg")}],
                                     "want": ["identify", "embed", "jpg", "aesthetics"],
