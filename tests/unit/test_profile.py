...
def test_builtin_profiles():
    ...
    assert album.want == ["identify", "embed", "quality", "scene"] and album.options["identify"]["species"] is False
    assert album.sources["identify"]["species"] == "profiles.toml" and album.plan.models == ("owlv2", "siglip2")
    assert album.plan.stages == ("embed", "identify", "quality", "scene") and album.reducers == ["burst", "select"]
...
def test_payload_with_a_profile(photos, tmp_path):
    ...
    assert body["want"] == ["identify", "embed", "quality", "scene"] and "profile" not in body
...
def test_eval_request():
    ...
    assert r == {"profile": "album", "want": ["identify", "embed", "quality", "scene"],
...
def test_eval_meta_line_records_the_profile(tmp_path, monkeypatch):
    ...
    assert sent[1]["want"] == ["identify", "embed", "quality", "scene"]
...
def test_config_show(tmp_path, capsys):
    ...
    assert d["stages"] == {"want": ["identify", "embed", "quality", "scene"], "from": "profiles.toml",
                           "run_order": ["embed", "identify", "quality", "scene"]}
...
def test_config_show_command_stays_import_light(tmp_path):
    ...
    assert "HEAVY []" in out and "run order embed, identify, quality, scene" in out and '"run_order"' in out
