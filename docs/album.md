...
- **Scene** (`scene`): SigLIP2 zero-shot over the frame vector the service already computes. The album profile
  ...
  keeps its 8 built-in labels, one per category. The landscape group also gets a horizon tilt (a `horizon_tilt` flag past 3°, never a reject).
