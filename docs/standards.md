...
`unit`, `how` and `source`, and optionally `tier` (when it is not the id's second segment, e.g.
`aesthetic-own`) and `profile` (the profile the tier runs under, e.g. `album`).
