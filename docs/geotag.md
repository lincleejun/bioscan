...
- **Clock offset** (camera time minus true time). It comes from the first of these that applies:
  ...
  The offset is **per camera** (EXIF Make + Model). In a folder of two or more cameras, each camera whose own photos include a clock photo or photos with GPS gets its own offset from them (steps 2-3 over that camera's photos); a camera with neither, and any photo without a Make/Model, uses the folder's offset (steps 2-3 over all photos, as for one camera). The summary on stderr adds one line per camera with its offset and where it came from. `--offset` holds for every camera; there is no per-camera `--offset`: give each camera a clock photo instead. A folder of one camera gets exactly what it got before. When most photos fall outside the track, a warning says how far off they are; a whole number of hours means a timezone mistake.
