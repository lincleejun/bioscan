...
| `budget-aesthetic.toml` | The budget for aesthetic golden-set reports (`bioscan bench aesthetic compare`): Spearman, keepers lost @20 %, precision@k, missing frames; pair, group and planted rules for sets that have them. Sized at about half of the EVA-100 run's 95 % intervals |
| `aes-golden-v1-eva-head-v1.json` | `bench aesthetic score` report (schema `bioscan-aesthetic-golden`) of the general head `eva-head-v1` on aes-golden-v1 = EVA-100 (`data/aesthetic/eva-golden-v1.csv`), owner's Mac, 2026-09-25. The first aesthetic baseline. `meta.golden` and `meta.scores` were made relative (`aes-golden-eva`, `runs/aes/eva-head-v1.ndjson`) and `meta.tier`/`profile` added; frame paths were already relative to the golden folder |
| `ci-album.json` | The album profile on the synthetic reject set made from 24 smoke photos (`tests/models`, `scripts/cull_synth.py`); `models.yml` compares every run with it under `budget-album.toml`. From run 36099637018 (2026-09-25, head 1a00598, with the inference cache: its speed is not real; cull metrics do not depend on it). Re-create like `ci-smoke.json`: delete it and `models.yml` prints the next candidate between `===== BEGIN bioscan-report ci-album candidate =====` markers |
...
Image paths inside a bench report are the machine's own (an aesthetic golden report keeps them relative to the golden folder); compare pairs images by sha256 first, so a baseline made on
