...
| `album` | identify, embed, quality, scene; reducers burst, select (run by `bioscan cull`, never the service) | identify `species: false`; scene labels; burst and select limits | SigLIP2, OWLv2 (BioCLIP never loads) | the `aesthetics` stage (C2), which select reads when present |
...
- **Accuracy**: unverified on real albums. CI measures the rules and reducers on a synthetic reject set made from the
  smoke photos (docs/harness.md "Album tier", docs/standards.md §13); XMP ratings or labels are not written yet.
...
bioscan/plugins/<name>/          built-in stages identify, embed, jpg, geotag, quality, scene: stdlib manifest in __init__.py,
                                 service code in stage.py; reducer manifests burst, select
