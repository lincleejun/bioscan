...
**Scene category** (album profile), 1,861 CC BY photos from Open Images V7 in 8 groups (`data/scene/scene-v1.csv`): group correct 75.0% overall with the album profile's 40 fine labels in 8 groups plus light, setting and framing attributes ([#34](https://github.com/lincleejun/bioscan/issues/34)): wildlife 93%, people 89%, food 80% (short prompts, [#45](https://github.com/lincleejun/bioscan/issues/45)), night 76%, architecture 74%, other 72%, landscape 68%, macro 47% (fungi and plants read as animals, [#46](https://github.com/lincleejun/bioscan/issues/46)). The old 8 labels gave 70.6%.
...
| **Score a folder**: aesthetic ranking exported as NDJSON, CSV and/or an HTML review page (taxon tree with merges, keep/drop marks, full-size lightbox), with the animals named on `--species`; re-export offline from the NDJSON; the page copies the keeps out and deletes the drops through the running service, or `aesthetic apply` does it from the saved decisions | `bioscan aesthetic score DIR -r --species --export json,csv,html --out aes` | [album.md](docs/album.md) |
...
| **HTTP API**: `/run` streams NDJSON; `/health`, `/products`, `/apply` (the review page's copy / delete) | `curl 127.0.0.1:8765/run` | [usage.md](docs/usage.md#http-api) |
