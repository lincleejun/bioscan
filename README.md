...
**Scene category** (album profile), 1,861 CC BY photos from Open Images V7 in 8 groups (`data/scene/scene-v1.csv`): group correct 70.6% overall; people 87%, food 88%, macro 86%, wildlife 77%; night, landscape, other and architecture 59–65% (measured with the old 8 labels, which are too coarse there). The album profile now names 40 fine labels in these 8 groups, plus light, setting and framing attributes ([#34](https://github.com/lincleejun/bioscan/issues/34)); its numbers are unverified until the scene tier is re-run.
...
| **Score a folder**: aesthetic ranking exported as NDJSON, CSV and/or an HTML gallery with thumbnails, with the animals named on `--species`; re-export offline from the NDJSON | `bioscan aesthetic score DIR -r --species --export json,csv,html --out aes` | [album.md](docs/album.md) |
